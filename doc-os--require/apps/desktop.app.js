define(['file.system/file.system', 'UI'], (FS, UI) => {
    ...
    function Desktop(os) {
        ...
        fs.addListener('desktop.app', payload => {
            ...
            if (payload.event === FS.CREATE_FILE) {
                if (payload.data.path !== this.path) return

                renderIcon(payload.data)
            }
            else if (payload.event === FS.CREATE_DIRECTORY) {
            ...
            //     }
            // }
            else if (payload.event === FS.DELETE) {
                if (!payload.data.path) return
